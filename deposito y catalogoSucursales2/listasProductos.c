...
void muestraLISTANoActivo(nodoProductos *lista)
{
    while (lista != NULL)
    {
        if (lista->dato.activoPro == 0)
        {
            mostrarUnProducto(lista->dato);
        }
        lista = lista->siguiente;
...
void muestraDeCatalogoNoActivo(catalogo A[],int validos)
{
    int i=0;
    ...
    {
        printf("n");
        printf("\nactivo: %d",A[i].activoCat);
        muestraUnCatalogoNoActivo(A[i]);
        i++;
    }
}
void muestraUnCatalogoNoActivo(catalogo A)
{
    if (A.activoCat == 0)
    {
        printf("\nNOMBRE DE CATEGORIA: %s", A.nombreDeCategoria);
